function build_trc_file(filename,outpath,markers)
...
%                to analyze. The output .trc file has the same name.
%     outpath (optional) = the path for the output file. If missing, it
%     is the same as the input file path.
%     markers (optional)  = cell array containing the names of markers
%                           to be included in the .trc file. If missing, all markers in
%                           OSM.osim are used (see line 18 below)
%
...
TRC_filename = fullfile(outpath,[name '.trc']);
...
for imark = 1:length(markers)
    ...
    if (isfield(data.marker_data.Markers,markers{imark}))
        ...
        % Vicon coordinate frame is x left, y back, z up. Convert to x
