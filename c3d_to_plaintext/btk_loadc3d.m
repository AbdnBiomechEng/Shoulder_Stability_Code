...
if ~isempty(fieldnames(angles))
    marker_data.Angles = angles;
    marker_data.AngleInfo = angleInfo;
end
...
% get force data
[forces,forceInfo] = btkGetForces(acq);
if ~isempty(fieldnames(forces))
    % write to data structure
    data.force_data.forces = forces;
    data.force_data.Info = forceInfo;
end
